...
def g_inference(x):
    ...
    with tf.variable_scope('g'):
        ...
        with tf.variable_scope('reshape'):
            w0 = tf.get_variable('weights', [100, i_depth[0] * 8 * 8], tf.float32,
        ...
        for i in range(4):
            with tf.variable_scope('conv%d' % (i + 1)):
                w = tf.get_variable('weights', [5, 5, o_depth[i], i_depth[i]], tf.float32,
                                    tf.truncated_normal_initializer(stddev=0.02))
                b = tf.get_variable('biases', [o_depth[i]], tf.float32, tf.zeros_initializer)
                dc = tf.nn.conv2d_transpose(out, w, [5104, 8 * 2 ** (i + 1), 8 * 2 ** (i + 1), o_depth[i]],
                                            [1, 2, 2, 1])
    ...
    with tf.variable_scope('d'):
        ...
        for i in range(4):
            with tf.variable_scope('conv%d' % i):
                w = tf.get_variable('weights', [5, 5, i_depth[i], o_depth[i]], tf.float32,
                                    tf.truncated_normal_initializer(stddev=0.02))
                b = tf.get_variable('biases', [o_depth[i]], tf.float32, tf.zeros_initializer)
...
# TODO: sess is being redefined, causes an error
